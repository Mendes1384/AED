...
void interpretador(Parque p){
	...
	do {
		...
		switch (cmd){
			case 'E': cmdEntradaAutomovel(p, args); break;
			case 'S': cmdSaidaAutomovel(p, args); break;
			case 'C': cmdDinheiro(p); break;
			case 'L': cmdListaAutomoveis(p); break;
			case 'X': break;
			case '\n': break;
