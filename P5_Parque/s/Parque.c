...
Parque criaParque (void){
    ...
    p->info = criaInfo("Parque", destroiParque, destroiTudoParque, NULL, NULL, NULL);
    p->caixa = 0;
...
/*Automovel posParque(Parque p, int i){
    return p->a[i];
}*/

Iterador iteradorParque(Parque p){
    return criaIterador((vetor)p->a, p->ocupacao);
}

/*Iterador iteradorOrdemDeChegadaParque(Parque p){
