...
static int slotInfo(string tipo) { // usa dispersao fechada
  ...
  int cap = CAPACIDADE;
  for (int i = d; i < cap; i++) {
    if (tabelaInfos[i] == NULL || strcmp(tabelaInfos[i]->tipo, tipo) == 0)
      return i;
  }
  for (int i = 0; i < d; i++) {
    if (tabelaInfos[i] == NULL || strcmp(tabelaInfos[i]->tipo, tipo) == 0)
...
static Info procuraInfo(string tipo) { return tabelaInfos[slotInfo(tipo)]; }
...
Info criaInfo(string tipo, void *destroi, void *destroiTudo, void *compara,
              void *dispersao, void *debug) {
...
void destroiInfo(Info info) {
  destroiString(info->tipo);
  free(info);
}

bool mesmoTipo(obj ob1, obj ob2) { return strcmp(tipo(ob1), tipo(ob2)) == 0; }

string tipo(obj ob) { return info(ob, "tipo")->tipo; }

void destroi(obj ob) { info(ob, "destroi")->destroi(ob); }

void destroiTudo(obj ob) { info(ob, "destroiTudo")->destroiTudo(ob); }
...
string debug(obj ob) { return info(ob, "debug")->debug(ob); }

void mostraObj(obj ob) { printf("<%s-%p>\n", tipo(ob), ob); }

/* ENGLISH */

Info createInfo(string type, void *destroy, void *destroyAll, void *compare,
                void *hash, void *debug) {
